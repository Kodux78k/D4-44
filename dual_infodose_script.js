...
// The embedded apps for each archetype. These definitions were
// previously injected into this file as standalone JSON and caused
// the script to break.  Defining them here and assigning to RAW.apps
// ensures a valid JavaScript structure.
const DEFAULT_APPS = [
...
// Remapeia a antiga aba 'revo' para 'chat'
    ...
    const RAW = { apps: [] };
    // Populate RAW.apps with the default catalog on load. This ensures the
    // PWA has a set of apps to render even if no embedded JSON is provided.
    RAW.apps = Array.isArray(DEFAULT_APPS) ? DEFAULT_APPS.slice() : [];

    // ===================== Archetype icon overrides (PNG) =====================
    // Some archetypes have dedicated PNG orb icons. The mapping below associates
    // the archetype key with a Base64-encoded PNG data URI. When available,
    // appIconFor will return these images instead of falling back to built‑in SVGs.
    ...
    function appIconFor(a){
      // Normalise the icon name. Accept `builtin:name` and strip the prefix.
      let name;
      if (!a || !a.icon) {
        name = 'atlas';
      } else {
        name = String(a.icon).replace(/^builtin:/, '');
      }
      // If we have a PNG override for this archetype, return it.
      if (ARCHETYPE_ICON_DATA[name]) {
        return ARCHETYPE_ICON_DATA[name];
      }
      // Use inline SVGs for recognised built‑in icons
      if (/^(atlas|nova|vitalis|pulse|artemis|serena|kaos|genus|lumine|rhea|solus|aion|local)$/.test(name)) {
        return svgIcon(name);
      }
      // Otherwise treat the icon as a path/URL
      return a.icon;
    }
